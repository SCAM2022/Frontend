...
const ShowEvent = () => {
  ...
  useEffect(() =>{
    const getEve = async() =>{
      const getEveData = await axios.post(`${process.env.REACT_APP_API_KEY}/fetchSingleEvent`,{
       ...
       var x = new Date(temp);
       setStime(finalStime = x.toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true }))
      }
    getEve();
  },[])
...
const onClickHandler = async() =>{
  ...
  const sendList = await axios.post(`${process.env.REACT_APP_API_KEY}/addParticipant`,{
    eveId: eveId,
  ...
  <h2 className="showEvent_heading">{details ? details.title: details}</h2>
  ...
  <span className="poster_info_color">{finalSdate}</span>
            ...
            <div className="showEvent_poster_info_right">
              ...
              <span>Contest Duration - {details ? details.timeDuration: details} hours</span>
            </div>
          </div>
        </div>
      ...
      <div className="showEvent_detail">
        ...
        <div className="showEvent_about">
          <h3>Description:</h3>
        <p>
        {details ? details.discription[0]: details}
        </p>
        </div>
        <div className="showEvent_about">
          <h3>Organizing Club:</h3>
          <p>
          {details ? details.createdBy: details}
          </p>
        </div>
        <div className="showEvent_contest_detail">
          <h3>Contest Detail</h3>
          <li><strong>Duration:</strong> {details ? details.timeDuration: details} hours</li>
          <li><strong>Start Date:</strong> {details ? details.startDate : details}</li>
          <li><strong>End Date:</strong> {details ? details.endDate : details} </li>
        </div>
        ...
        <div className="showEvent_about">
          ...
          <p>
          {details ? details.eventIncharge: details}
          </p>
        </div>
